...
#include <linux/config.h>
 ...
 static char * sg_version_str = "Version: 3.0.19 (20010623)";
#endif
 static int sg_version_num = 30019; /* 2 digits for each component */
/*
...
static Scsi_Cmnd * dummy_cmdp;    /* only used for sizeof */
                           ...
                           unsigned char * cmnd, int timeout, int blocking)
{
    ...
    SCpnt = scsi_allocate_device(NULL, sdp->device, blocking);
...
static int sg_write_xfer(Sg_request * srp)
{
    ...
    if ((SG_DXFER_TO_DEV == dxfer_dir) || (SG_DXFER_UNKNOWN == dxfer_dir) ||
...
static int sg_read_xfer(Sg_request * srp)
{
    ...
    if ((SG_DXFER_FROM_DEV == dxfer_dir) || (SG_DXFER_UNKNOWN == dxfer_dir) ||
        (SG_DXFER_TO_FROM_DEV == dxfer_dir)) {
...
static char * sg_low_malloc(int rqSz, int lowDma, int mem_src, int * retSzp)
{
    ...
    if (SG_HEAP_POOL == mem_src) {
        ...
        printk("sg_low_malloc: bad mem_src=%d, rqSz=%d\n", mem_src, rqSz);
    return resp;
}
    ...
    for (j = 0; j < max_dev; ++j) {
        ...
        if (sdp && (scsidp = sdp->device) && (! sdp->detached))
            PRINT_PROC("%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\n",
               ...
               (int)scsidp->queue_depth, 0, (int)scsidp->online);
        else
...
static int sg_proc_devhdr_info(char * buffer, int * len, off_t * begin,
                               off_t offset, int size)
{
    PRINT_PROC("host\tchan\tid\tlun\ttype\tbopens\tqdepth\tn/a\tonline\n");
    return 1;
}
